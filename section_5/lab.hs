...
toDigits n
    | n < 0 = error "Only positive numbers are supported"
    | n == 0 = []
...
doubleSecond xs = map (\x -> if even (snd x) then fst x else double x) xis
    where 
        xis = [(x,y) | (x, y) <- zip xs [0..]]
        double (x, _) = x * 2
...
isValid n = if sumDigits (doubleSecond (toDigitsRev n)) `mod` 10 == 0 then True else False
